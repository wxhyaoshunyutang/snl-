...
public:
    ...
    void add_node2(Syntax_tree* t)
    {
        child.insert(child.begin(), t);
    }
...
public:
    void syntax_error(string error)//语法报错
    {
        ...
        digui.push_back("syntax_error!! in line " + to_string(now_token->Line) + " from " + error + "\n");
        //exit(0);//发现错误直接退出先
        error_flag = 1;
    }
    TreeNode* match_token(string lex)//继续往后读Token序列,同时建立一个终极符的语法树节点
    {
        ...
        if (now_token->Lex == lex)
        {
            ...
            if (!error_flag)
            {
                ...
                digui.push_back(to_string(now_token->Line) + "行成功匹配" + " now_token->Lex: " + now_token->Lex + "\n");
            }
     
            now_token = &token[token_index++];
            if (now_token->Lex == "EOF" && !error_flag)
            {
                    cout << "语法分析成功，无错误" << endl;
                    digui.push_back("语法分析成功，无错误\n");
            }
